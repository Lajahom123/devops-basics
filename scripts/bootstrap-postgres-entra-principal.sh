...
RUNTIME_DIR="$ROOT_DIR/infra/runtime-aks"
SQL_FILE="$RUNTIME_DIR/bootstrap-postgres-entra-principal.sql"
...
check_dns_resolution() {
  ...
  if command -v getent >/dev/null 2>&1; then
    ...
    dig +short "$host" >/dev/null 2>&1 && [[ -n "$(dig +short "$host")" ]] && return 0
...
require_command terraform "Install Terraform in this execution environment."
...
[[ -f "$SQL_FILE" ]] || fail "SQL file not found at '$SQL_FILE'. Run this script from the repository checkout or include infra/runtime-aks/bootstrap-postgres-entra-principal.sql."
